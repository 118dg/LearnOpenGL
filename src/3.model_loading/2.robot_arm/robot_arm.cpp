...
//const char* ourObjectPath = "C:/LearnOpenGL/src/3.model_loading/2.robot_arm/zephyr_duck.obj"; //방법1. 절대경로로 넣거나
const char* ourObjectPath = "./zephyr_duck.obj"; //방법2. bin폴더 안에 zephyr_duck.obj파일을 넣고 상대경로로 넣거나
//const char* ourObjectPath = "./teapot.obj";

// translate it so it's at the center of the scene
// it's a bit too big for our scene, so scale it down
glm::mat4 objectXform = glm::rotate(glm::rotate(glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(0.5f, 0.1f, 0.0f)), glm::vec3(0.0008f, 0.0008f, 0.0008f)), glm::radians(-150.0f), glm::vec3(0.0, 1.0, 0.0)), glm::radians(-90.0f), glm::vec3(1.0, 0.0, 0.0));
...
void myDisplay()
{
	...
	model = glm::rotate(model, glm::radians(BaseSpin), glm::vec3(0.0, 1.0, 0.0));
	DrawBase(model); //Base
	model = glm::translate(model, glm::vec3(0.0f, 0.4f, 0.0f));
	model = glm::rotate(model, glm::radians(ShoulderAng), glm::vec3(0.0, 0.0, 1.0));
	DrawJoint(model); //Joint
	DrawArmSegment(model); //Shouler
	model = glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f));
	model = glm::rotate(model, glm::radians(ElbowAng), glm::vec3(0.0, 0.0, 1.0));
	DrawJoint(model); //Joint
	DrawArmSegment(model); //Elbow
	model = glm::translate(model, glm::vec3(0.0f, 0.5f, 0.0f));
	model = glm::rotate(model, glm::radians(WristAng), glm::vec3(0.0, 0.0, 1.0));
	model = glm::rotate(model, glm::radians(WristTwistAng), glm::vec3(0.0, 1.0, 0.0));
	DrawWrist(model); //Wrist
	...
	model1 = glm::rotate(model1, glm::radians(FingerAng1), glm::vec3(0.0, 0.0, 1.0));
	DrawFingerBase(model1);
	model1 = glm::translate(model1, glm::vec3(0.0f, 0.35f, 0.0f));
	model1 = glm::rotate(model1, glm::radians(FingerAng2), glm::vec3(0.0, 0.0, 1.0));
	DrawFingerTip(model1);

	//손가락 2
	model2 = glm::translate(model2, glm::vec3(0.0f, 0.2f, 0.0f));
	model2 = glm::rotate(model2, glm::radians(-FingerAng1), glm::vec3(0.0, 0.0, 1.0));
	DrawFingerBase(model2);
	model2 = glm::translate(model2, glm::vec3(0.0f, 0.35f, 0.0f));
	model2 = glm::rotate(model2, glm::radians(-FingerAng2), glm::vec3(0.0, 0.0, 1.0));
	DrawFingerTip(model2);

	//Teapot	
	//if(flag_space) {
	//	objectXform = glm::scale(model, glm::vec3(0.08f, 0.08f, 0.08f));
	//}
...
void processInput(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (key >= GLFW_KEY_1 && key <= GLFW_KEY_5 && action == GLFW_PRESS)
		...
		if (flag_space) flag_space = false;
	...
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS && !blinnKeyPressed)
	{
		blinn = !blinn;
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (LeftButtonDown)
	{
		switch (RobotControl)
		{
		case 0: xoffset /= SCR_WIDTH; yoffset /= SCR_HEIGHT; BaseTransX += xoffset; BaseTransZ -= yoffset; break;
		case 1: xoffset /= SCR_WIDTH; yoffset /= SCR_HEIGHT; BaseSpin += xoffset * 180; break;
		case 2: xoffset /= SCR_WIDTH; yoffset /= SCR_HEIGHT; ShoulderAng += yoffset   * -90; ElbowAng += xoffset  * 90; break;
		case 3: xoffset /= SCR_WIDTH; yoffset /= SCR_HEIGHT; WristAng += yoffset  * -180; WristTwistAng += xoffset  * 180; break;
		case 4: xoffset /= SCR_WIDTH; yoffset /= SCR_HEIGHT; FingerAng1 += yoffset  * 90; FingerAng2 += xoffset * 180; break;
		}
	} 
...
Plane::Plane()
{
	...
	//floorTexture = loadTexture("C:/LearnOpenGL/src/3.model_loading/2.robot_arm/wood.png"); //방법1. 절대 경로로 하거나
	floorTexture = loadTexture("./wood.png"); //방법2. bin폴더에 wood.png파일을 넣고 상대 경로로 하거나
	FloorShader->use();
